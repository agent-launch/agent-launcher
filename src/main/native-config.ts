 ...
 *   - Gemini:   settings.json selects API-key auth and disables gemini-cli's
 *               own silent self-update. The relay endpoint and key stay in
 *               env vars.
 */
...
/** Selects the profile's API-key auth and forces off gemini-cli auto-update. */
function mergedGeminiSettings(existing: Record<string, any>): Record<string, any> {
  const profile = getActiveProfile('gemini')
  const security = objectValue(existing.security)
  ...
  return {
    ...
    ...(profile?.apiKey?.trim()
      ? {
          security: {
            ...security,
            auth: { ...auth, selectedType: 'gemini-api-key' }
          }
        }
      : {}),
    general: {
