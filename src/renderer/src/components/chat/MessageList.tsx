...
import { Brain, Check, ChevronDown, CircleCheck, CircleX, Copy, Loader2, TerminalSquare, Wrench } from 'lucide-react'
...
function partText(part: TranscriptPart): string {
  if (part.kind === 'tool') {
    return [
      `[${part.tool ?? 'tool'}] ${part.detail ?? ''}`.trim(),
      part.input,
      part.result
    ].filter(Boolean).join('\n\n')
  }
...
const MessageBlock = memo(function MessageBlock({
  ...
  const label =
    msg.role === 'system' ? t('transcript.role.system') : assistantName || t('transcript.role.assistant')
  ...
  return (
    <div className="group/message flex gap-3">
      <span className="grid size-7 shrink-0 place-items-center overflow-hidden rounded-md text-[11px] font-medium text-text-weak">
        {assistantCliId && msg.role !== 'system' ? <CliIcon cliId={assistantCliId} size={15} /> : label.slice(0, 1)}
      </span>
...
function toolVerbKey(tool: string | undefined): string {
  ...
  if (['write', 'edit', 'multi_edit', 'apply_patch'].includes(name)) return 'transcript.toolVerb.edit'
  if (['bash', 'shell', 'exec', 'exec_command', 'command_execution'].includes(name)) return 'transcript.toolVerb.run'
  ...
  return (
    <details className="group/tool rounded-lg border border-border-weak bg-surface-weak/55">
      <summary className="flex min-w-0 cursor-pointer select-none items-center gap-2 px-3 py-2 text-[13px] text-text-weak transition-colors hover:text-text-strong">
        ...
        {resultPreview && <span className="hidden min-w-0 truncate text-text-base md:inline">{resultPreview}</span>}
        <span className="ml-auto shrink-0">{statusIcon}</span>
        <ChevronDown size={14} className="shrink-0 transition-transform group-open/tool:rotate-180" />
      </summary>
      <div className="space-y-3 border-t border-border-weak px-3 py-3">
        {part.input && (
          <ToolDetailBlock label={t('transcript.toolInput')} text={part.input} />
        )}
        {part.result && (
          <ToolDetailBlock
...
function ToolDetailBlock({ label, text, danger }: { label: string; text: string; danger?: boolean }) {
  return (
    <div>
      <div className={`mb-1.5 text-[11px] font-medium ${danger ? 'text-danger' : 'text-text-weak'}`}>
        {label}
      </div>
