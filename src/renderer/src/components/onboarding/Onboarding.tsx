...
function DetectStep() {
  ...
  return (
    <StepShell
      title={t("onboarding.detectTitle")}
      desc={t("onboarding.detectDesc")}
    >
      {!result ? (
        <div className="text-[13px] text-text-weak">
        ...
        <>
          <ul className="space-y-2">
            {result.items.map((it) => {
              const cliId = CLIS.some((c) => c.id === it.key)
                ? (it.key as CliId)
                : null;
  ...
  const modal = (
    ...
    >
      <div className="space-y-4">
        ...
        {cleanupError && (
          <div className="text-[12px] text-danger">{cleanupError}</div>
        )}
        <div className="flex justify-end gap-2">
          ...
          <Button disabled={cleanupBusy} onClick={confirmCleanup}>
            {cleanupBusy
              ? t("onboarding.cleanupBusy")
              : t("onboarding.cleanupConfirm")}
          </Button>
        </div>
      </div>
    </Modal>
...
interface CliPathManagerProps {
  ...
  setCleanupTarget: (
    target: { cliId: CliId; candidate: SystemCliCandidate } | null,
  ) => void;
      ...
      <div className={layout === "detect" ? "hidden" : "contents"}>
        {detected.duplicate && (
          <Button
            ...
            variant="ghost"
            onClick={() =>
              setExpanded((prev) => ({ ...prev, [cliId]: !prev[cliId] }))
            }
          >
            {isExpanded
              ? t("onboarding.collapsePaths")
              : t("onboarding.expandPaths")}
          </Button>
        )}
      </div>
                  ...
                  {selected ? (
                    ...
                    <div className="flex shrink-0 items-center gap-2 text-[12px]">
                      <button
                        ...
                        onClick={() =>
                          useCandidate(cliId, candidate, onInstallUi)
                        }
...
function InstallStep() {
  ...
  const [systemClis, setSystemClis] = useState<
    Partial<Record<CliId, SystemCliDetection>>
  >({});
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([window.api.config.get(), refreshDetection()]).then(
      async ([cfg, result]) => {
        if (cancelled) return;
                ...
                >
                  {s.error ? (
                    ...
                    <span style={{ color: "var(--success)" }}>
                      {t(
                        s.source === "system"
                          ? "onboarding.systemLinked"
                          : "onboarding.installed",
                        {
                          version:
                            s.version && s.version !== "installed"
                              ? ` ${s.version}`
                              : "",
                        },
                      )}
...
function ConfigStep() {
  ...
  const startLogin = async (
    id: CliId,
    method: "official" | "device" = "official",
  ) => {
    setCliId(id);
    setMode("official");
          ...
          >
            <div className="flex items-center gap-2">
              <span className="truncate text-[13px] font-medium text-text-strong">
                {t(
                  cliId === "codex"
                    ? "onboarding.authCardCodex"
                    : "onboarding.authCardClaude",
                )}
              </span>
  ...
  return (
    <div className="mt-4 rounded-xl border border-border-weak bg-surface/92 p-4 shadow-[var(--shadow-sm)]">
      <div className="flex items-center gap-3">
        ...
        {authBusy ? (
          ...
          >
            {t(
              cliId === "codex"
                ? "onboarding.authLoginCodex"
                : "onboarding.authLoginClaude",
            )}
          </Button>
        )}
      ...
      <div className="mt-3 flex items-center gap-3">
        <Button onClick={onUseOfficial}>
          {t("onboarding.authUseOfficial")}
        </Button>
        {saved && (
          <span className="text-[13px] text-success">
            {t("onboarding.saved")}
...
function StepShell({
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h2 className="font-display text-[22px] font-semibold text-text-strong">
        {title}
      </h2>
      <p className="mt-2 mb-6 text-[14px] leading-relaxed text-text-base">
        {desc}
      </p>
      {children}
    </div>
