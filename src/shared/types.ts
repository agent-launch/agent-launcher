...
/** RecentProject plus main-process-only facts the renderer can't compute. */
export interface RecentProjectInfo extends RecentProject {
  /** False when the directory no longer exists on disk (moved/deleted). */
  exists: boolean
}
...
export interface CliLinkProgress {
  cliId: CliId
  /** 'install' | 'native' | 'npm' | 'official' | 'mirror' only occur while installing a
   * CLI the user does not have yet; linking never reports them. */
  phase: 'link' | 'verify' | 'install' | 'native' | 'npm' | 'official' | 'mirror' | 'done' | 'error'
  message: string
}
