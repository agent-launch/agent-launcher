...
describe('provider model discovery', () => {
  ...
  it('only accepts an explicit model endpoint on the Base URL origin', () => {
    ...
    ).toBe('https://relay.example/openai/v1/models')
    expect(
      modelDiscoveryUrl(
        'claude-code',
  ...
  it('filters OpenAI-compatible model lists for Gemini', () => {
    expect(
      parseModelDiscoveryResponse(
        {
          data: [
  ...
  it('uses Bearer auth for a Gemini OpenAI-compatible model endpoint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response('{"data":[{"id":"gpt-test"},{"id":"gemini-2.5-pro"}]}', {
        status: 200
      })
    )
    vi.stubGlobal('fetch', fetchMock)
    ...
    await expect(
      discoverModels('gemini', {
        ...
        apiKey: 'gemini-key',
        modelsUrl: 'https://relay.example/api/v1/models'
      })
    ).resolves.toEqual({
      ok: true,
      code: 'ok',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      new URL('https://relay.example/api/v1/models'),
      expect.objectContaining({
  ...
  it('falls back from Gemini native discovery to an OpenAI-compatible model list', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(new Response('{"detail":"Not Found"}', { status: 404 }))
      .mockResolvedValueOnce(
        new Response('{"data":[{"id":"gpt-test"},{"id":"gemini-3.7-flash"}]}', {
          status: 200
        })
      )
    vi.stubGlobal('fetch', fetchMock)
    ...
    await expect(
      ...
      code: 'ok',
      status: 200,
      models: [{ id: 'gemini-3.7-flash', name: 'gemini-3.7-flash' }]
    })
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      new URL('https://relay.example/api/v1beta/models?key=gemini-key'),
      expect.anything()
    )
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
