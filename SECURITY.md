...
Use [GitHub private vulnerability reporting](https://github.com/agent-launch/agent-launcher/security/advisories/new).

请使用 [GitHub 私密漏洞报告](https://github.com/agent-launch/agent-launcher/security/advisories/new)。

If that form is unavailable, open an issue that contains only a request for a private channel — no version details, no reproduction steps, no logs — and a maintainer will follow up to arrange one. Do not describe the vulnerability in that issue.

如果该表单不可用，请提交一个仅说明“需要私密渠道沟通安全问题”的 Issue——不要写版本细节、复现步骤或日志——维护者会跟进并安排渠道。不要在该 Issue 中描述漏洞本身。
