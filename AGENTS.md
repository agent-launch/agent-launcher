...
Reads each CLI's **own** on-disk conversation history so users can resume — each CLI stores it differently: Codex/Codex/Pi use JSONL (different dir layouts and title fields), **opencode uses a SQLite DB read via `sql.js`/WASM**. `resumeArgs()` maps a session id back to the CLI's resume flag. Adding a CLI means adding both a `list*` reader and a `resumeArgs` case.
