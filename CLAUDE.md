...
The app ships from the public `agent-launch/agent-launcher` repo, and that identity is load-bearing: the appId `app.agent-launch.agentlauncher` keys macOS permissions and preferences, and the owner/repo in `electron-builder.yml`, `src/main/app-update.ts`, and `update-policy.json` is the auto-update feed every install polls. `tests/repo/identity.test.ts` pins all of it — including that `.github/workflows/release.yml` always publishes with `prerelease: true`, so promoting a build to the one updater clients actually receive stays a manual step (`releases/latest` skips prereleases). That gate covers the release feed only: `app-update.ts` polls a **second** channel, `update-policy.json` read from `main` over `raw.githubusercontent.com`, and its `latestVersion` (plus `force`) drives the in-app update banner whether or not the release was promoted. Bump that file only after promoting, or the gate is bypassed. Release artifact names must survive a GitHub upload verbatim: `artifactName` deliberately avoids `${productName}` — its space is rewritten to a dot when the workflow uploads the asset, while electron-builder writes a dash into `latest*.yml`, and electron-updater downloads by that exact yml name, 404ing every auto-update (v0.1.0 and v0.2.0-rc.1 shipped broken this way). The identity test and the workflow's _Verify update metadata_ step both pin this.

`mac.forceCodeSigning: true` turns a missing signing certificate into a hard failure instead of a silently unsigned build. CI pull-request builds are unaffected — electron-builder skips signing on PRs before the flag is consulted — but a local `pnpm package` / `pnpm package:mac` now needs a Developer ID identity in the keychain; `identity: null` and `CSC_IDENTITY_AUTO_DISCOVERY=false` both still throw, so the only local workaround is to drop the flag. On CI the certificate is imported into a dedicated keychain by `release.yml`'s _Import macOS signing certificate_ step and passed through `CSC_KEYCHAIN`; `CSC_LINK` is deliberately unset for macOS because electron-builder's own `createKeychain` unlocks its temporary keychain with the `.p12` password instead of the keychain password and fails on a locked keychain. On CI the blast radius is wider than macOS: the `release` job needs every build job, so an unset `MAC_CSC_LINK` blocks the Windows and Linux releases for that tag too — `release.yml`'s _Require macOS signing credentials_ step fails fast and says so instead of dying inside electron-builder. Windows is deliberately left soft (no `forceCodeSigning`) until `WIN_CSC_LINK` exists.
...
2. **Native config files** — `src/main/native-config.ts` writes the files some CLIs read instead of (or in addition to) env: Claude Code `settings.json` (env block), Codex `config.toml`+`auth.json`, opencode `opencode.json` (custom `@ai-sdk/openai-compatible` provider), Pi `models.json`, Hermes `config.yaml`+`.env`. `hasNativeConfig(id)` gates this. Gemini's provider/auth config stays env-var only, but it also gets a `settings.json` write that forces off gemini-cli's own silent self-update (`general.enableAutoUpdate`/`enableAutoUpdateNotification`) — that update mid-launch restarts the process, which can trip gemini-cli's own exit-time cleanup of "not resumable" sessions and has been observed deleting/overwriting real session files. **Any profile change in `ipc.ts` re-runs `writeNativeConfig` via the `synced()` wrapper** — keep that invariant when adding config mutations.
